...
from numba import jit, int8, int16, float32, void, types
...
class DiehlAndCook2015Network:
    ...
    def __call__(self, s_in, stdp=True):
        ...
        s_exc = self.exc_neurons(self.delay_input(g_in), self.g_inh*2e-2)
        c_exc = self.exc_synapse(s_exc)
        g_exc = np.dot(self.exc2inh_W, c_exc)
        x_exc = self.exc_synaptictrace(s_exc)
        
        
        s_inh = self.inh_neurons(self.delay_exc2inh(g_exc*1e-1), 0)
        ...
        if stdp:
            ...
            if self.tcount == self.update_nt:
                ...
                clipped_dW = np.clip(dW / self.update_nt, -5e-3, 5e-3)
...
@jit   
def train(dt=1e-3, t_inj=0.35, t_blank=0.15, n_neurons=100,
          n_labels=10, n_epoch=10, N_train=300, n_minibatch=1):
    nt_inj = round(t_inj/dt)
    nt_blank = round(t_blank/dt)
    
    update_nt = nt_inj*n_minibatch
    train, _ = chainer.datasets.get_mnist()
    _, labels = load_and_encoding_dataset(train, N_train, dt, nt_inj, max_fr=20)

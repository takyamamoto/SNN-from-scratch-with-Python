...
class DiehlAndCook2015Network:
    def __init__(self, N_in=784, N_neurons=100, wexc=0.225, winh=0.175,
                 dt=1e-3, wmin=0.0, wmax=0.1, lr=(1e-4, 1e-2),
                 update_nt=100):
        """
        ...
        self.inh_neurons = ConductanceBasedLIF(N_neurons, dt=dt, tref=2e-3,
                                               ...
                                               e_exc=0, e_inh=-85)
        # Synapses
        self.input_presynapse = SingleExponentialSynapse(N_in, dt=dt, td=2e-2)
        self.exc_presynapse = SingleExponentialSynapse(N_neurons, dt=dt, td=1e-2)
        
        self.input_postsynapse = SingleExponentialSynapse(N_neurons, dt=dt, td=2e-2)
        self.exc_postsynapse = SingleExponentialSynapse(N_neurons, dt=dt, td=2e-2)
        self.inh_postsynapse = SingleExponentialSynapse(N_neurons, dt=dt, td=2e-2)
        """
        self.input_presynapse = SingleExponentialSynapse(N_in, dt=dt, td=2e-2)
        self.exc_presynapse = SingleExponentialSynapse(N_neurons, dt=dt, td=1e-2)
        ...
        self.input_conn = FullConnection(N_in, N_neurons,
                                         initW=1e-4*np.random.rand(N_neurons, N_in))
    ...
    def initialize_states(self):
        ...
        self.input_presynapse.initialize_states()
        self.exc_presynapse.initialize_states()
        self.input_postsynapse.initialize_states()
        self.exc_postsynapse.initialize_states()
        self.inh_postsynapse.initialize_states()
        
    def __call__(self, s_in, stdp=True, update=False):
        c_in = self.input_presynapse(s_in)

        index_in = np.where(s_in)[0] # 発火したニューロンのindex
        len_idx_in = len(index_in)
        #print("len_idx_in", len_idx_in)
        if len_idx_in > 0:
            self.JD_in = np.sum(self.input_conn.W[:, index_in], axis=1)      
        g_in = self.input_postsynapse(self.JD_in*(len_idx_in> 0))

        s_exc = self.exc_neurons(self.delay_input(g_in), self.g_inh)
        print(s_exc)
        c_exc = self.exc_presynapse(s_exc)
         # この時点で同じ出力？
        
        index_exc = np.where(s_exc)[0] # 発火したニューロンのindex
        len_idx_exc = len(index_exc)
        #print("len_idx_exc", len_idx_exc)
        if len_idx_exc > 0:
            self.JD_exc = np.sum(self.exc2inh_W[:, index_exc], axis=1) #.astype(np.float64)
        g_exc = self.exc_postsynapse(self.JD_exc*(len_idx_exc> 0))
        #print("self.JD_exc", self.JD_exc)
        s_inh = self.inh_neurons(g_exc, 0)
        #s_inh = self.inh_neurons(self.delay_exc2inh(g_exc), np.zeros(self.N_neurons))
        
        index_inh = np.where(s_inh > 0)[0] # 発火したニューロンのindex
        len_idx_inh = len(index_inh)
        #print("len_idx_inh", len_idx_inh)
        if len_idx_inh > 0:
            self.JD_inh = np.sum(self.inh2exc_W[:, index_inh], axis=1)      
        self.g_inh = self.exc_postsynapse(self.JD_inh)
        ...
        if stdp:
            ...
            # Online STDP
            if self.tcount == self.update_nt - 1:
                W = self.input_conn.W
                #dW = self.lr_p*np.matmul(self.s_exc_, self.c_in_)
                #dW -= self.lr_m*np.matmul(self.c_exc_, self.s_in_)
                dW = self.lr_p*(self.wmax - W)*np.matmul(self.s_exc_, self.c_in_)
                dW -= self.lr_m*W*np.matmul(self.c_exc_, self.s_in_)
                self.dW += dW
                self.reset_trace()
                if update:
                    dW = np.clip(self.dW, -1e-3, 1e-3)
                    self.input_conn.W = np.clip(self.input_conn.W+dW,
                                                self.wmin, self.wmax)
                    self.dW = np.zeros((self.N_neurons, self.N_in)) # reset
                    
                    #w_abs_sum = np.expand_dims(np.sum(np.abs(W), axis=0), 0)
                    #w_abs_sum[w_abs_sum == 0] = 1.0
                    #W = W * self.norm / w_abs_sum
                    #self.input_conn.W = W / W.max() * 
        else:
            if self.tcount > 0:
                self.reset_trace()

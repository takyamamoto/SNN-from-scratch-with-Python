...
def prediction(spikes, assignments, n_labels):
    ...
    return np.argmax(rates, axis=1).astype(np.uint8) # (n_neuroms, )
...
class DiehlAndCook2015Network:
    ...
    def __call__(self, s_in, stdp=True):
        ...
        s_exc = self.exc_neurons(self.delay_input(g_in), self.g_inh*1e-2)
        c_exc = self.exc_synapse(s_exc)
        g_exc = np.dot(self.exc2inh_W, c_exc)
        x_exc = self.exc_synaptictrace(s_exc)
        
        
        s_inh = self.inh_neurons(self.delay_exc2inh(g_exc*0.1), 0)
...
spikes = np.zeros((N_train, nt_inj, n_neurons)).astype(np.uint8)
    ...
    for i in tqdm(range(N_train)):
        ...
        while(True):
            ...
            for t in range(nt_inj):
                ...
                #voltage[i, t] = network.exc_neurons.v_
            
            spikes[i] = np.array(spike_list)

...
class DiehlAndCook2015Network:
    def __init__(self, N_in=784, N_neurons=100, wexc=22.5, winh=17.5,
                 dt=1e-3, wmin=0.0, wmax=1.0, lr=(1e-4, 1e-2)):
        """
    ...
    def __call__(self, s_in, stdp=True):
        ...
        self.tcount += 1
        if stdp:
            s_in_ = np.expand_dims(s_in, 0)
            s_exc_ = np.expand_dims(s_exc, 1)
            c_in_ = np.expand_dims(c_in, 0) 
            c_exc_ = np.expand_dims(c_exc, 1)

            # Online STDP
            W = self.input_conn.W
            dW = self.lr_p*(self.wmax - W)*np.matmul(s_exc_, c_in_)
            dW -= self.lr_m*W*np.matmul(c_exc_, s_in_)
            self.input_conn.W = np.clip(W+dW, self.wmin, self.wmax)
